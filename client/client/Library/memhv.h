...
namespace HV
{
	...
	inline ULONG64 GetProcessBase(ULONG64 targetEprocess)
	{
		return CallVM(Shared::MAGIC, Shared::CommandId::GetProcessBaseAddress, targetEprocess, HV::Data::TargetDirectoryBase);
	}

	inline bool PlaceHook(ULONG64 FunctionToHook, ULONG64 HandlerFunction) {

		Shared::HOOK_DATA data = { 0 };
		data.TargetFunctionDirectoryBase = HV::Data::TargetDirectoryBase;
		data.FunctionToHook = FunctionToHook;
		data.HandlerFunctionDirectoryBase = HV::Data::TargetDirectoryBase;
		data.HandlerFunction = HandlerFunction;

		std::cout << "Handler Function : " << std::hex << data.HandlerFunction << std::dec << std::endl;
		std::cout << "Function to Hook : " << std::hex << data.FunctionToHook << std::dec << std::endl;
		std::cout << "Handler Function Directory Base : " << std::hex << data.HandlerFunctionDirectoryBase << std::dec << std::endl;
		std::cout << "Target Function Directory Base : " << std::hex << data.TargetFunctionDirectoryBase << std::dec << std::endl;

		auto output = CallVM(Shared::MAGIC, Shared::CommandId::CreateHook, reinterpret_cast<ULONG64>(&data), HV::Data::CurrentDirectoryBase);
